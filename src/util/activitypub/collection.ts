...
import { addContext, WithContext } from "./util.js";
...
type Props<T extends BaseModel> = {
	...
	keys?: Extract<keyof T, string | number>[];
	id: string;
...
export const collectionHandler = async <T extends BaseModel>(
	props: Props<T>,
): Promise<WithContext<APCollection>> => {
	...
	return addContext({
		...
		type: props.ordered ? "OrderedCollection" : "Collection",
		next: next ? next.toString() : undefined,
		prev: prev ? prev.toString() : undefined,

		totalItems: await qb.getCount(),
		[props.ordered ? "orderedItems" : "items"]: data.map(convert),
	});
};
