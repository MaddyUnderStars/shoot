...
import { signWithHttpSignature } from "../util/activitypub/httpsig.js";
...
export const sendActivity = async (
	...
	const inboxes = targets.reduce<Set<string>>((ret, target) => {
		...
		// If we have a shared inbox, and this activity is going to another actor with the same shared inbox, use that
			...
			shared_inbox &&
			targets.filter((x) => x.collections?.shared_inbox === shared_inbox).length > 1
		)
	...
	for (const inbox of inboxes) {
		const signed = signWithHttpSignature(inbox, "POST", sender, JSON.stringify(activity));
		...
		} else Log.verbose(`Sent activity ${activity.id} got response`, await res.text());
	}
};
